...
public class SelectionExpandCommand extends GoldenAppleCommand {
    ...
    public boolean onExecute(GoldenApple instance, User user, String commandLabel, String[] args) {
        ...
        if (args.length < 1 || args.length > 3) {
            return false;
        } else if (!user.hasPermission(SelectManager.builtinExpandPermission)) {
        ...
        if ((amount < 0 || reverseAmount < 0) && !user.hasPermission(SelectManager.builtinContractPermission)) {
            GoldenApple.logPermissionFail(user, commandLabel, args, true);
            return true;
        }
