...
public class AreaManager {
	 ...
	 * @param onlyParentAreas If set to false, will allow ChildAreas in the list. If true, will only include the ChildArea's parent.
	 */
	...
	/**
	 * Verifies that an Area is one that is stored in this official
	 * areas HashMap.
	...
	/**
	 * Assembles and returns a List of all stored ChildrenAreas
	 * who's parent is the given ParentArea
	...
	/**
	 * This method does <strong>NOT</strong> actually delete the ParentArea and keep
	 * the children. <strong>INSTEAD</strong>, the ParentArea will inherit corner1,
	 * corner2, and ignoreY from its first child, then delete the
	 * first child. <u>If the ParentArea does not have any children, then
	 * it will delete itself.</u>
	 * @param ID The ID of the ParentArea to "delete"
	...
	public ParentArea getParentArea(Long ID){
		if(areas.containsKey(ID)
				&& areas.get(ID) instanceof ParentArea)
			return (ParentArea) areas.get(ID);
		return null;
	}
	
	/**
	 * This method sorts a ParentArea's children's indexes such that
	 * their indexes will start at 0 and go up sequentially from there
	 * in order WITHOUT skipping over any number. This method preserves
	 * the numerical order of the children's indexes.
	 ...
	 * @param includeChildren If true, will include ChildAreas who's parents are instances of PrivateAreas
	...
	public List<PrivateArea> getAllPrivateAreas(boolean includeChildren){
		...
		for(Area area : areas.values())
			...
			else if (includeChildren && area instanceof ChildArea
					&& ((ChildArea) area).getParent() instanceof PrivateArea)
	 ...
	 * @param includeChildren If true, will include ChildAreas who's parents are instances of PvpAreas
		...
		for(Area area : areas.values())
			if (area instanceof PvpArea)
				pvpAreas.add((PvpArea) area);
					...
					&& ((ChildArea) area).getParent() instanceof PvpArea)
	 ...
	 * @param includeChildren If true, will include ChildAreas who's parents are instances of SafetyAreas
		...
		for(Area area : areas.values())
			if (area instanceof SafetyArea)
				safetyAreas.add((SafetyArea) area);
					...
					&& ((ChildArea) area).getParent() instanceof SafetyArea)
	 ...
	 * @param includeChildren If true, will include ChildAreas who's parents are instances of TownAreas
		...
		for(Area area : areas.values())
			if (area instanceof TownArea)
				townAreas.add((TownArea) area);
					...
					&& ((ChildArea) area).getParent() instanceof TownArea)
