...
public class PunishmentBan extends Punishment {

	public PunishmentBan(ResultSet r) throws SQLException {
	...
	public PunishmentBan(IPermissionUser target, IPermissionUser admin, String reason, RemainingTime duration) {
	...
	public boolean update() {
		try {
			GoldenApple.getInstance().database.execute("UPDATE Bans SET Target=?, Admin=?, Reason=?, StartTime=?, Duration=?, Voided=? WHERE ID=?",
	...
	public boolean insert() {
		try {
			ResultSet r = GoldenApple.getInstance().database.executeReturnGenKeys("INSERT INTO Bans (Target, Admin, Reason, StartTime, Duration, Voided) VALUES (?, ?, ?, ?, ?, ?)",
