...
public class PermissionsCommand implements CommandExecutor {
	public boolean onCommand(CommandSender sender, Command command, String commandLabel, String[] args) {
		...
		GoldenApple.getInstance().locale.sendMessage(user, "header.permissions", false);
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equalsIgnoreCase("-u")) {
				if (i == args.length - 1 || args[i + 1].startsWith("-")) {
					GoldenApple.getInstance().locale.sendMessage(user, "shared.parameterMissing", false, args[i]);
				} else {
				...
				if (add) {
					GoldenApple.getInstance().locale.sendMessage(user, "error.permissions.conflict", false);
					return true;
		...
		if (!remove && !add && addPermissions.isEmpty() && remPermissions.isEmpty() && addUsers.isEmpty() && remUsers.isEmpty() && addGroups.isEmpty() && remGroups.isEmpty()) {
			GoldenApple.getInstance().locale.sendMessage(user, "error.permissions.noAction", false);
			return true;
		}
		if (remove) {
			...
			for (String u : changeUsers) {
				long id = GoldenApple.getInstance().permissions.getUserId(u);
				if (id != -1 && !GoldenApple.getInstance().permissions.isSticky(id))
					users.add(id);
			}
			for (String g : changeGroups) {
				PermissionGroup group = GoldenApple.getInstance().permissions.getGroup(g);
				if (group != null)
					groups.add(group.getId());
			}
			if (users.isEmpty() && groups.isEmpty()) {
				GoldenApple.getInstance().locale.sendMessage(user, "error.permissions.noTarget", false, "-d");
				return true;
			}
			if (verified) {
