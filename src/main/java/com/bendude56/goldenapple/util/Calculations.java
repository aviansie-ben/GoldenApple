...
/**
 * This class will contain various functions and methods
 * that are important to Minecraft plugins. A repository
 * of miscellaneous calculations that are less specialized
...
public class Calculations {
	 ...
	 * @param height Set to true if you want to take the y-coordinate into account.
	 * Set to false to ignore the Y-coordinate.
	 * @return The distance between the two entities.
	 */
	public double getDistance(Entity entity1, Entity entity2, boolean height) {
	 ...
	 */
	public double getDistance(Location location1, Location location2, boolean height) {
		if (height) {
			return getDistance(location1.getX(), location1.getY(), location1.getZ(),
					location2.getX(), location2.getY(), location2.getZ());
		} else {
			return getDistance(location1.getX(), 0, location1.getZ(),
					location2.getX(), 0, location2.getZ());
		}
	}
